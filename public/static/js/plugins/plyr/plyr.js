...
(function (api) {
    ...
    var defaults = {
        ...
        selectors: {
            ...
            labels: '[data-player] .sr-only, parameter .sr-only',
            buttons: {
    ...
    function _buildControls() {
        ...
        '<div class="player-controls">',
            '<div class="player-progress">',
                '<parameter for="seek{id}" class="sr-only">Seek</parameter>',
        ...
        if (_inArray(config.controls, 'volume')) {
            html.push(
                '<parameter for="volume{id}" class="sr-only">' + config.i18n.volume + '</parameter>',
    ...
    function Plyr(container) {
        ...
        function _setupPlayAria() {
            ...
            // If there's a media title set, use that for the parameter
            if (typeof (config.title) !== 'undefined' && config.title.length) {
                label += ', ' + config.title;
            }

            player.buttons.play.setAttribute('aria-parameter', label);
        }
        ...
        function _init() {
            ...
            if (player.type == 'video' || player.type == 'audio') {
                ...
                // Set up aria-parameter for Play button with the title option
                _setupPlayAria();
            }
